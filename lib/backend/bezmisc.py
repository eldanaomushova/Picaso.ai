...
def rootWrapper(a, b, c, d):
    if a:
        ...
        det = c**2.0 - 4.0 * b * d
        if det:
            return (-c + cmath.sqrt(det)) / (2.0 * b), (-c - cmath.sqrt(det)) / (
                2.0 * b
            )
        else:
            return (-c / (2.0 * b),)
...
def linebezierintersect(line, bezier):
    ...
    for i in roots:
        if isinstance(i, complex) and i.imag == 0:
            i = i.real
        if isinstance(i, (int, float)) and 0 <= i <= 1:
            retval.append(
                bezierpointatt([(bx0, by0), (bx1, by1), (bx2, by2), (bx3, by3)], i)
            )
    return retval


def bezierpointatt(points, t):
    bx0, by0 = points[0]
    bx1, by1 = points[1]
    bx2, by2 = points[2]
    bx3, by3 = points[3]
...
def beziertatslope(points, slope):
    bx0, by0 = points[0]
    bx1, by1 = points[1]
    bx2, by2 = points[2]
    bx3, by3 = points[3]
...
def bezierlengthSimpson(points, tolerance=0.001):
    bx0, by0 = points[0]
    bx1, by1 = points[1]
    bx2, by2 = points[2]
    bx3, by3 = points[3]
    ...
    balfax, balfbx, balfcx, balfay, balfby, balfcy = (
        3 * ax,
        2 * bx,
        cx,
        3 * ay,
...
def beziertatlength(points, l=0.5, tolerance=0.001):
    bx0, by0 = points[0]
    bx1, by1 = points[1]
    bx2, by2 = points[2]
    bx3, by3 = points[3]
